...
ipcMain.handle('restart-computer', async () => {
  try {
    ...
    if (result.response === 0) {
      exec(
        `shutdown /r /t 30 /c "Restarting computer via ${branding.productName} - Type '${restartDialog.cancelCommand}' to cancel"`,
        (error) => {
          if (error) {
      ...
      return {
        success: true,
        message: `Computer will restart in 30 seconds. ${restartDialog.cancelNote}`,
      };
    } else {
...
ipcMain.handle('run-system-file-checker', async () => {
  try {
    ...
    return {
      success: true,
      message: 'System File Checker started (admin approval may be required)',
    };
  } catch (error) {
