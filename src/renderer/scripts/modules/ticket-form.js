export class TicketForm {
  ...
  setupDragAndDrop() {
    ...
    dragArea.addEventListener('click', (e) => {
      if (
        e.target.id !== 'takeScreenshotBtn' &&
        window.zsAttachedAttachmentsCount < this.config.attachments.maxCount
      ) {
        const nextAttachment = document.getElementById(
          'zsattachment_' + window.zsAttachmentFileBrowserIdsList[0]
        );
        if (nextAttachment) {
  ...
  handleFileSelection(files) {
    for (
      let i = 0;
      i < files.length && window.zsAttachedAttachmentsCount < this.config.attachments.maxCount;
      i++
    ) {
      const file = files[i];
...
Data Collected: ${new Date().toLocaleString()}`;
  ...
  attachScreenshot({ bytesBase64, filename, mimeType }) {
    ...
    if (window.zsAttachedAttachmentsCount >= maxCount) {
      this.toast.showMessage(
        `Max ${maxCount} attachments — remove one to add screenshot`,
        'warning'
      );
      return;
    }
  ...
  updateScreenshotsDisplay() {
    ...
    if (this.state.currentScreenshots.length === 0) {
      container.innerHTML =
        '<div class="no-screenshots"><div class="screenshot-icon">📷</div><div class="screenshot-text">No screenshots</div></div>';
    } else {
      const screenshotItems = this.state.currentScreenshots
        .map(
          (screenshot, i) =>
            '<div class="screenshot-item">' +
            '<div class="screenshot-preview">' +
            '<img src="' +
            (screenshot.preview?.dataUrl || 'data:image/png;base64,' + screenshot.preview?.base64) +
            '" alt="Screenshot ' +
            (i + 1) +
            '" />' +
            '</div>' +
            '<div class="screenshot-info">' +
            '<div class="screenshot-name">Screenshot ' +
            (i + 1) +
            '</div>' +
            '<div class="screenshot-details">' +
            (screenshot.dimensions?.width || 'Unknown') +
            'x' +
            (screenshot.dimensions?.height || 'Unknown') +
            '</div>' +
            '</div>' +
            '</div>'
        )
