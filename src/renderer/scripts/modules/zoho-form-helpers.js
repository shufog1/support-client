// Zoho WebToCase form boilerplate — generated by Zoho, isolated here for clarity.
// Must be loaded as a regular (non-module) script AFTER jqueryandencoder.js.
// All functions are globals used by the Zoho form's own event wiring and by ticket-form.js.

function trimBoth(str) {
  return jQuery.trim(str);
}
// setAllDependancyFieldsMapping uses window.__zohoFormId (set by app.js from config)
function setAllDependancyFieldsMapping() {
  var fid = window.__zohoFormId || '5211000000795236';
  ...
  if (mapDependancyLabels) {
    for (var i = 0; i < mapDependancyLabels.length; i++) {
      ...
      var obj = document.forms['zsWebToCase_' + fid][label];
      if (obj) {
  ...
  if (val != null && val != '' && val != 'null' && mySelObject) {
    ...
    for (var i in fields) {
      if (fields.hasOwnProperty(i)) {
        ...
        var depObj = document.forms['zsWebToCase_' + fid][label];
...
// Limit sourced from config at boot — this initial value is overridden by app.js
// via window.zsAllowedAttachmentLimit = config.attachments.maxCount (see app.js)
var zsAllowedAttachmentLimit = 5;
...
jQuery(document)
  ...
  .on('click.fba', '.zsfilebrowseAttachment', function () {
    ...
    jQuery('#zsattachment_' + currentlyDeletedElement).replaceWith(
...
function zsChangeMousePointer() {
  if (zsAttachedAttachmentsCount >= 5) {
    ...
    jQuery('#dragDropArea').css('cursor', 'default');
  } else {
...
// zsResetWebForm uses window.__zohoFormId (set by app.js from config) instead of a hardcoded value
function zsResetWebForm(webFormId) {
  var fid = webFormId || window.__zohoFormId || '';
  document.forms['zsWebToCase_' + fid].reset();
  document.getElementById('zsSubmitButton_' + fid).removeAttribute('disabled');
